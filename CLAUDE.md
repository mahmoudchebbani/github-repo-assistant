...
- **No API written from memory.** Every pgvector, fastembed, LangGraph, dlt and psycopg call is
  checked against the installed package or current documentation before use. LangGraph especially —
  it is the one library here with no prior line written.
...
1. `make check` passes — ruff lint, ruff format check, pytest. Paste the output, do not summarise it.
...
- [`code-style.md`](.claude/rules/code-style.md) — `**/*.py`. The simplicity contract.
- [`testing.md`](.claude/rules/testing.md) — `tests/**`. What earns a test here, and what does not.

Two agents in `.claude/agents/`:

- `implementer` — builds a task, verifies it, commits, reports.
- `code-reviewer` — read-only. Reviews correctness first and clarity second, and treats unnecessary
  abstraction as a defect in this project.
