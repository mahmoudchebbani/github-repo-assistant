...
| *Best practice* — hybrid search | 1 | pgvector cosine and Postgres full-text search, both legs in [`assistant/search.py`](assistant/search.py) |
| *Best practice* — re-ranking | 1 | `reciprocal_rank_fusion` in [`assistant/search.py`](assistant/search.py) — written here, not imported |
...
what ships rather than sitting in a notebook.
...
Everything below was measured during the build, not guessed.
...
2. **The LLM evaluation's metrics saturate.** Both prompts scored 5.00 relevance and effectively
...
3. **The lexical leg is Postgres full-text search, not BM25.** It has no document-length
...
4. **HNSW is approximate.** The dense leg reads an index that can miss a true nearest neighbour, so
...
5. **A refusal phrased entirely in the model's own words is not recognised as a refusal.** Citation
...
6. **Citations are matched by the literal `[label]` form.** A source the model names any other way —
...
7. **`repo=None` searches every repository in the index, not every repository in `REPOS`.** Removing
   ...
   = …`.
8. **Indexing rebuilds rather than diffs.** `make index` deletes and re-embeds every chunk for a
...
9. **Grafana's latency and cost panels carry a visible "Last 6 hours" override** and do not follow
   the dashboard time picker. Changing the range at the top moves the other three panels only.
10. **Chunk text is truncated at 512 word pieces by the embedding model, silently.** Measured on
...
| `test_agent.py` | with an empty index the answer is the refusal, and the model is never called |
| `test_db.py` | a conversation round-trips, and `cost_usd` comes back as an exact `Decimal` |
| `test_smoke.py` | end to end: a raw record → chunk → embed → index → retrieved with its citation intact |
